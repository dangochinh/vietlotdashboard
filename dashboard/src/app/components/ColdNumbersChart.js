...
export default function ColdNumbersChart({ data, onViewAll }) {
    ...
    return (
        <div className="bg-gray-900/40 rounded-3xl p-6 border border-gray-800 shadow-xl flex flex-col">
            <div className="flex items-center justify-between mb-6">
                ...
                <button
                    onClick={() => onViewAll({ open: true, title: 'Tất Cả Số Khan', type: 'cold' })}
            ...
            <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data.top15} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        />
                        <Bar dataKey="drawsAbsent" radius={[4, 4, 0, 0]}>
                            {data.top15.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={index < 3 ? CHART_COLORS.cold.top : CHART_COLORS.cold.rest} />
                            ))}
                        </Bar>
                    </BarChart>
                </ResponsiveContainer>
